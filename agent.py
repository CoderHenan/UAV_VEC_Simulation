...
class ST_MASAC_Agent(BaseAgent):
    ...
    # [修改] 接收 adj 参数, 返回 attention weights
    def select_action(self, obs_list, adj, noise=False):
        ...
        # 返回: action, attention_weights, dummy...
    ...
    def update(self, transition):
        ...
        for _ in range(utd):
            ...
            adj_t = torch.FloatTensor(adj).to(self.device)  # (Batch, N, N)
...
class DDPG_Agent(BaseAgent):
    ...
    def select_action(self, obs_list, adj, noise=False):  # adj 占位
        actions = []
        for i, actor in enumerate(self.actors):
    ...
    def update(self, transition):
        # 即使 DDPG 不用 adj，为了接口统一也要存
        self.memory.push(*transition)
        if len(self.memory) < cfg.BATCH_SIZE: return

        batch, _, _ = self.memory.sample(cfg.BATCH_SIZE)
        states, actions, rewards, next_states, dones, _, _, _, _, _ = batch  # 忽略 adj
        for i in range(cfg.N_UAV):
...
class QLearning_Agent(BaseAgent):
    ...
    def load_ckpt(self, model_path, csv_path=None):
        if os.path.exists(os.path.join(model_path, "q_table.pkl")):
            with open(os.path.join(model_path, "q_table.pkl"), "rb") as f:
                data = pickle.load(f)
